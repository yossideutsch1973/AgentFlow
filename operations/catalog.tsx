...
export const operationCatalog: Record<OpType, OperationDefinition> = {
  ...
  [OpType.EXPERT_PROMPT_ENGINEER]: {
    ...
    defaultInputs: { prompt: undefined, system: undefined },
    defaultParams: {
      model: 'gemini-2.5-flash',
      out: 'text',
      temperature: 0.7,
      provider: 'google',
      system: 'You are an expert in prompt engineering. Your task is to refine the given prompt to be more effective for a large language model. Focus on clarity, conciseness, and providing sufficient context for the model to generate a high-quality response. Do not generate the code, only the prompt.',
    },
  },
  [OpType.EXPERT_CODE_OPTIMIZER]: {
    type: OpType.LLM,
    label: 'Code Optimization Expert',
    paletteClasses: 'bg-teal-500/20 text-teal-300 hover:bg-teal-500/30 ring-1 ring-teal-500/30',
    nodeBorderClass: 'border-teal-500',
    icon: makeIcon(<path strokeLinecap="round" strokeLinejoin="round" d="M8.25 3v1.5M4.5 8.25H3m18 0h-1.5M4.5 12H3m18 0h-1.5m-15 3.75H3m18 0h-1.5M8.25 19.5V21M12 3v1.5m0 15V21m3.75-18v1.5m0 15V21m-9-1.5h10.5a2.25 2.25 0 0 0 2.25-2.25V8.25a2.25 2.25 0 0 0-2.25-2.25H6.75A2.25 2.25 0 0 0 4.5 8.25v7.5A2.25 2.25 0 0 0 6.75 18z" />),
    inputHandles: [
    ...
    defaultParams: {
      ...
      temperature: 0.7,
      provider: 'google',
      system: 'You are an expert in code optimization. Your task is to refine the given prompt to include instructions for generating optimized and efficient code. Focus on best practices, performance, and readability. Do not generate the code, only the prompt.',
    },
  },
  [OpType.EXPERT_CLARITY_CONCISENESS]: {
    type: OpType.LLM,
    label: 'Clarity and Conciseness Expert',
    paletteClasses: 'bg-teal-500/20 text-teal-300 hover:bg-teal-500/30 ring-1 ring-teal-500/30',
    nodeBorderClass: 'border-teal-500',
    icon: makeIcon(<path strokeLinecap="round" strokeLinejoin="round" d="M8.25 3v1.5M4.5 8.25H3m18 0h-1.5M4.5 12H3m18 0h-1.5m-15 3.75H3m18 0h-1.5M8.25 19.5V21M12 3v1.5m0 15V21m3.75-18v1.5m0 15V21m-9-1.5h10.5a2.25 2.25 0 0 0 2.25-2.25V8.25a2.25 2.25 0 0 0-2.25-2.25H6.75A2.25 2.25 0 0 0 4.5 8.25v7.5A2.25 2.25 0 0 0 6.75 18z" />),
    inputHandles: [
    ...
    defaultParams: {
      ...
      temperature: 0.7,
      provider: 'google',
      system: 'You are an expert in clarity and conciseness. Your task is to refine the given prompt to be as clear and concise as possible, without sacrificing important details. Focus on making the prompt easy to understand and unambiguous. Do not generate the code, only the prompt.',
    },
  },
  [OpType.LOOP]: {
    type: OpType.LOOP,
